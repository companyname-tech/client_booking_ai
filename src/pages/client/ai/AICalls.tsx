...
export default function AICalls() {
  ...
  const allCalls = campaigns.flatMap((c) => repo.getCalls(c.id))
  const allLeads = campaigns.flatMap((c) => repo.getLeads(c.id))
  const { filters, setFilters, filtered, summary } = useCallFilters(allCalls)
  const [selected, setSelected] = useState<Call | null>(null)
  const campaign = selected ? campaigns.find((c) => c.id === selected.campaignId) : undefined
  const detail = selected && campaign ? repo.getCall(campaign.id, selected.id) : null
  const lead = selected ? allLeads.find((l) => l.id === selected.leadId) : undefined
  const conversation = selected ? repo.getAIConversations().find((c) => c.callId === selected.id) : undefined
  const convDetail = conversation ? repo.getAIConversation(conversation.id) : undefined
